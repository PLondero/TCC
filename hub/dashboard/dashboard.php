...
<html lang="pt-BR">
            ...
            <section class="dashboard">
                ...
                <div class="boxes">
                    ...
                    <div class="box" id="box2">
                        ...
                        <h1> 
                            <?php
                                $sql = "SELECT SUM(valor) AS total FROM valores_fixos WHERE id_usuario = {$_SESSION['id_usuario']} AND valor < 0";
    ...
    <script>
        ...
        const receitas = <?php echo $conn->query("SELECT saldo FROM usuarios WHERE id_usuario={$_SESSION["id_usuario"]}")->fetch_assoc()["saldo"]?>;

        const dadosReceita = xValues.map((m, i) => i === mesAtual ? receitas : 0);

        const despesas = <?php $sql = "SELECT SUM(valor) AS total FROM transacao 
            WHERE id_usuario = {$_SESSION['id_usuario']} AND valor < 0";
            $result = $conn->query($sql);
            $row = $result->fetch_assoc();
            $total = $row['total'] ?? 0;
            echo $total;
        ...
        const dadosGastos = xValues.map((m, i) => i === mesAtual ? despesas : 0);
...
<?php
    $result = $conn->query("SELECT razao, valor FROM transacao WHERE id_usuario={$_SESSION["id_usuario"]} AND valor < 0");
    $listavalores = "";
    $listarazao = "";
    while($row = $result->fetch_assoc()){
        $listavalores.= $row["valor"]*-1 . ",";
        $listarazao.= "\"{$row["razao"]}\"" . ",";
    } 

?>

<script>
    const razaodespesas = [<?php 
        echo substr($listarazao, 0, -1)
    ?>]

    const valor = [<?php 
        echo substr($listavalores, 0, -1)
    ?>]
